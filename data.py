...
class DrugNovoLoader(object):
    def __init__(self,
        ...
        self._generate_feat()
        self._load_llm_features()
        self.possible_rel_values = self.values

    def _load_llm_features(self):
        drug_llm_path = os.path.join(f'./raw_data/drug_data/{self._name}/', 'drug_llm_emb.npy')
        dis_llm_path = os.path.join(f'./raw_data/drug_data/{self._name}/', 'disease_llm_emb.npy')

        drug_llm = np.load(drug_llm_path)
        dis_llm = np.load(dis_llm_path)

        self.drug_llm_feature = drug_llm.astype(np.float32)
        self.disease_llm_feature = dis_llm.astype(np.float32)
    ...
    def _generate_topoy_graph(self):
        ...
        for cv in range(0, len(self.row_idx)):
            ...
            self.train_enc_graph = self._generate_enc_graph(train_pairs, train_values,
                                                            add_support=True)
            self.subgraphs = self._generate_hetero_subgraphs(self.train_enc_graph, num_subgraphs=self.num_sub)
    ...
    def _generate_feat_graph(self):
        ...
        drug_edge_weights = drug_sim[dr_row_index, dr_col_index]
        drug_adj = sp.coo_matrix(
            (drug_edge_weights, (drug_edges[:, 0], drug_edges[:, 1])),
        ...
        disease_edge_weights = disease_sim[di_row_index, di_col_index]
        disease_adj = sp.coo_matrix(
            (disease_edge_weights, (disease_edges[:, 0], disease_edges[:, 1])),
    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False):
        ...
        if add_support:
            ...
            for etype in extra_edge_types:
                src_type, rel_type, dst_type = graph.to_canonical_etype(etype)
                if src_type == 'drug' and dst_type == 'drug':
                    drug_ci.append(graph[etype].in_degrees())
                    if self._symm:
                        drug_cj.append(graph[etype].out_degrees())
                    else:
                        drug_cj.append(th.zeros((self.num_drug,)))
                elif src_type == 'disease' and dst_type == 'disease':
                    disease_ci.append(graph[etype].in_degrees())
                    if self._symm:
                        disease_cj.append(graph[etype].out_degrees())
                    else:
                        disease_cj.append(th.zeros((self.num_disease,)))
    ...
    def _generate_hetero_subgraphs(self, enc_graph, num_subgraphs=3):
        ...
        edges_per_subgraph_dict = {}
        for etype in edge_dict:
            num_edges = len(edge_dict[etype][0])
            edges_per_subgraph = num_edges // num_subgraphs
            perm = th.randperm(num_edges)
            edge_dict[etype] = (edge_dict[etype][0][perm], edge_dict[etype][1][perm])
            edges_per_subgraph_dict[etype] = edges_per_subgraph

        for i in range(num_subgraphs):
            data_dict = {}

            for etype in edge_dict:
                edges_per_subgraph = edges_per_subgraph_dict[etype]
