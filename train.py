...
def train(args, dataset, graph_data, cv):
    ...
    drug_llm_feat = th.from_numpy(dataset.drug_llm_feature).float().to(args.device)
    dis_llm_feat = th.from_numpy(dataset.disease_llm_feature).float().to(args.device)
    args.llm_dim = drug_llm_feat.shape[1]
    # build the model
    model = SEDR(args=args)
    model.drug_llm_feat = drug_llm_feat
    model.dis_llm_feat = dis_llm_feat
    ...
    for iter_idx in range(1, args.train_max_iter):
        model.train()
        Two_Stage = False
        pred_ratings, drug_out, dis_out, llm_drug, llm_dis  = \
            model(train_dec_graph, drug_feat, dis_feat,subgraphs, Two_Stage)

        pred_ratings = pred_ratings.squeeze(-1)

        loss_com_drug = common_loss(drug_out, llm_drug)
        loss_com_dis = common_loss(dis_out, llm_dis)
        ...
        auroc, aupr, f1, precision, recall, y_true, y_score = evaluate(args, model, graph_data, drug_sim_feat, dis_feat, subgraphs)
    ...
    print(
        "Bset_Iter={}, Best_AUROC={:.4f}, Best_AUPR={:.4f}, Best_F1={:.4f}, Best_precision={:.4f}, Best_recall={:.4f}".format(
            best_iter, best_auroc, best_aupr, best_f1, best_precision, best_recall))
    test_loss_logger.close()
    return best_auroc, best_aupr, best_f1, best_precision, best_recall


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='SEDR')
    ...
    parser.add_argument('--num_sub', type=int, default=4)
    ...
    for times in range(1, 11):
        ...
        dataset = DrugDataLoader(args.data_name, args.device, args.num_sub,
        ...
        print("Mean_AUROC{:4f}".format(np.mean(auc_list)), "Mean_AURP{:4f}".format(np.mean(aupr_list)),
              "Mean_F1{:4f}".format(np.mean(f1_list)), "Mean_precision{:4f}".format(np.mean(precision_list)),
              "Mean_recall{:4f}".format(np.mean(recall_list)))
